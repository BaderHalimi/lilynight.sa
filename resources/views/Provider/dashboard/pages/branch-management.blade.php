...
<div x-data="{ open: false }" id="branchModal" x-on:open-branch-modal.window="open = true" class="relative" x-cloak>
    ...
    <div
        ...
        <div class="flex flex-col space-y-1.5 text-center sm:text-left">
            ...
            <p id="modal-desc" class="text-sm text-muted-foreground">أدخل بيانات الفرع أو الموقع الجديد لتبدأ بإدارته وربطه بخدماتك.</p>
        </div>
        ...
        <div class="py-4 max-h-[60vh] overflow-y-auto pr-2">
            <div class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2" for="branchName">اسم الفرع/الموقع</label>
                        <input id="branchName" name="name" placeholder="مثال: قاعة الياسمين للاحتفالات" class="flex h-10 w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 transition-all duration-200 ease-in-out" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2" for="branchLocation">عنوان الفرع/الموقع</label>
                        <input id="branchLocation" name="location" placeholder="مثال: الرياض، شارع العليا" class="flex h-10 w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 transition-all duration-200 ease-in-out" />
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2" for="branchManager">اسم المدير المسؤول</label>
                        <input id="branchManager" name="manager" placeholder="مثال: عبدالله السالم" class="flex h-10 w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 transition-all duration-200 ease-in-out" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2" for="branchPhone">رقم هاتف الفرع</label>
                        <input id="branchPhone" name="phone" placeholder="05xxxxxxxx" class="flex h-10 w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 transition-all duration-200 ease-in-out" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 mt-2 p-3 border rounded-md">
                        <template x-for="service in ['قاعات','بوفيه','تصوير','تجميل','ترفيه','نقل','أمن','زهور ودعوات']" :key="service">
                            <div class="flex items-center gap-2">
                                <input id="services" type="checkbox" :id="'service-' + service" class="form-checkbox h-4 w-4 text-primary border-slate-300 rounded focus:ring-primary">
                                <label :for="'service-' + service" class="block font-medium text-gray-700 mb-2 text-sm" x-text="service"></label>
                            </div>
                        </template>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input id="Branchemail" name="Branchemail" class="w-full p-2 border border-slate-300 rounded-md focus:ring-primary focus:border-primary">

                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2" for="branchStatus">حالة الفرع</label>
                    <select id="branchStatus" name="status" class="w-full p-2 border border-slate-300 rounded-md focus:ring-primary focus:border-primary">
        ...
        <div class="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 gap-2 pt-4 border-t">
            <button @click="open = false" class="inline-flex items-center justify-center rounded-lg text-sm font-semibold ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 hover:bg-accent hover:text-accent-foreground h-10 px-4 py-2">
                إلغاء
            </button>
            <button id="saveBranch" class="inline-flex items-center justify-center rounded-lg text-sm font-semibold bg-primary hover:bg-primary/90 shadow-md h-10 px-4 py-2 text-white">
                إضافة الفرع
            </button>
        </div>
    </div>
...
document.getElementById("saveBranch").addEventListener("click", async function(e) {
    ...
    const saveBtn = this;
    const editId = saveBtn.getAttribute("data-edit-id"); // هل فيه id تعديل؟
    ...
    try {
        ...
        if (editId) {
            ...
            res = await apiPut(route("provider.Dashboard.ProviderBranches.update", editId), payload);
        } else {
        ...
        document.getElementById("branchesTable").addEventListener("click", async (e) => {
            ...
            if (btn.classList.contains("editBtn")) {
                try {
                    const branch = await apiGet(route("provider.Dashboard.ProviderBranches.edit", id));
                    console.log(branch);
                    ...
                    document.querySelectorAll('input[type="checkbox"]').forEach(cb => {
                        cb.checked = branch.services?.includes(cb.nextElementSibling.textContent) || false;
                    });
                    window.dispatchEvent(new Event("open-branch-modal"));
                    //document.querySelector('#branchModal').__x.$data.open = true;
